      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-4xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-purple-400">Configuración</h1>
          </div>
          <button className="flex items-center gap-2 bg-slate-800/50 border border-cyan-500/30 rounded-lg px-4 py-2 text-gray-300" onClick={load}>
            <RefreshCw className="w-5 h-5 text-cyan-400" />Actualizar
          </button>
        </div>
          ...
          <div className="p-4 flex flex-wrap gap-2">
            {filteredPayments.length === 0 ? (
              ...
              filteredPayments.map(p => (
                <span key={p} className="px-3 py-1 rounded-full border border-cyan-500/40 text-cyan-300 bg-slate-900/40">{p}</span>
              ))
            )}
          </div>
        </div>
          ...
          <div className="p-4 flex flex-wrap gap-2">
            {filteredStates.length === 0 ? (
              ...
              filteredStates.map(s => (
                <span key={s} className="px-3 py-1 rounded-full border border-purple-500/40 text-purple-300 bg-slate-900/40">{s}</span>
              ))
            )}
          </div>
        </div>
